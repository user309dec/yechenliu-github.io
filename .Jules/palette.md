...
## 2026-08-05 - JS prefers-reduced-motion fallback
...
**Action:** Always include a `window.matchMedia("(prefers-reduced-motion: reduce)").matches` check when writing custom JavaScript animation loops or intervals, providing an immediate fallback state.
